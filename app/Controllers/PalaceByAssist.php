...
class PalaceByAssist extends BaseController
{
	...
	public function exportExcel()
	{
		...
		$data['DataReq'] = $this->getDataReq($_GET['hID'],$_GET['directiveType']);
	...
	public function getDataReq($hID,$directiveType)
	{
		$arr_data = array();
		...
		$builder->join("DataPersonalForcesMap AS t2","t1.positionMapID = t2.positionMapID AND t2.typeForce = '1'","left");
		$builder->join("DataPersonalForces AS t3","t2.fid= t3.fid","left");
		$builder->join("DataPersonalForcesMapHead AS t4","t2.hID = t4.id AND t4.directiveType = '{$directiveType}'","left");
		if($directiveType == 1){
			$builder->where("t2.hID = '{$hID}'");
		}else{
			$builder->where("t2.hIDRetire = '{$hID}'");
		}
		$builder->orderBy("t1.org_id ASC,t1.positionMapID ASC");
		$result = $builder->get()->getResult();
		// echo $db->getLastQuery();
	...
	public function getDataSendDirective()
	{
		...
		$builder->select("t3.pid,
						...
						t4.directiveNo AS no_cmd_for_out,
						DATE(t2.dateBegin) AS date_cmd_in_for_out,
						DATE(t2.dateEnd) AS date_out,
		...
		if(!empty($result)){
			foreach($result AS $key=>$value){
				...
				if($value->type_cmd == 'I'){
					...
					$date_out = "";
				}else if($value->type_cmd == 'IO'){
					$date_in =$value->date_in;
					$date_out = $value->date_out;
				}
				$arr_persons[$key]['date_in'] = $date_in;
				$arr_persons[$key]['no_cmd_for_out'] = $no_cmd_for_out;
				$arr_persons[$key]['date_cmd_in_for_out'] = $date_cmd_in_for_out;
				$arr_persons[$key]['date_out'] = $date_out;
	...
	//ส่งไปออกคำสั่งขอพ้น
	public function getDataSendRetire()
	{
		$hID = $this->request->getPost('hID');
		$arr_data = array();
		$db = db_connect();
		...
		$builder->select("t3.pid,
						...
						t3.hrTypeID AS hr_type,
						DATE(t2.dateBegin) AS date_in,
						t5.directiveNo AS no_cmd_for_out,
						DATE(t2.dateBegin) AS date_cmd_in_for_out,
						DATE(t2.dateEnd) AS date_out,
						IF(t4.orderTypeID = '1','I',IF(t4.orderTypeID='2','O',IF(t4.orderTypeID='5','IO',''))) AS type_cmd,
						t3.positionCivilianID AS personalPositionCivilianID");
		$builder->join("DataPersonalForcesMap AS t2","t1.positionMapID = t2.positionMapID AND t2.typeForce = '1'","left");
		$builder->join("DataPersonalForces AS t3","t2.fid= t3.fid","left");
		$builder->join("DataPersonalForcesMapHead AS t4","t2.hIDRetire = t4.id AND t4.directiveType = 2","left");
		$builder->join("DataPersonalForcesMapHead AS t5","t2.hID = t5.id AND t5.directiveType = 1","left");
		$builder->where("t2.hIDRetire = '{$hID}'");
		$builder->orderBy("t1.org_id ASC,t1.positionMapID ASC");
		$result = $builder->get()->getResult();
		// echo $db->getLastQuery();
		...
		if(!empty($result)){
			foreach($result AS $key=>$value){
				...
				if($value->type_cmd == 'O'){
					$no_cmd_for_out = $value->no_cmd_for_out;
					$date_cmd_in_for_out = $value->date_cmd_in_for_out;
					$date_out = $value->date_out;
				}
				$arr_persons[$key]['date_in'] = $date_in;
				$arr_persons[$key]['no_cmd_for_out'] = $no_cmd_for_out;
